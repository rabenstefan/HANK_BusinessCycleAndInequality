function incomes(n_par,m_par,distr,N,r,w,profits,RL,mcw,q,τprog,τlev,Ht,H)
    ...
    inc[1:5] =[  ((GHHFA .* τ_lev.*(n_par.mesh_y ./H) .^tax_prog_scale .* mcw .*w .*N ./ Ht).^(1.0 .- τ_prog)).+
        ((1.0 .- mcw) .* w .*N .*(1.0 .- av_tax_rate)),# labor income (NEW)
        (r .- 1.0) .* n_par.mesh_k, # rental income
        eff_int .* n_par.mesh_m, # liquid asset Income
        n_par.mesh_k .* q, # capital liquidation Income (q=1 in steady state)
        τ_lev.*(mcw .*w .*N .*n_par.mesh_y ./H).^(1.0-τ_prog).*((1.0 - τ_prog)/(m_par.γ+1))] 
    inc[1][:,:,end].= τ_lev.*(n_par.mesh_y[:,:,end] .* profits).^(1.0-τ_prog)  # profit income net of taxes
    inc[5][:,:,end].= 0.0
